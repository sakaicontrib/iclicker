...
public interface IClickerDao extends GeneralGenericDao {
    ...
    /**
     * Allows a lock to be obtained that is system wide, this is primarily for ensuring something
     * runs on a single server only in a cluster<br/>.
    ...
    /**
     * Releases a lock that was being held, this is useful if you know a server is shutting down and you
     * want to release your locks early<br/>
     * <b>NOTE:</b> This intentionally returns a null on failure rather than an exception since exceptions
     * will cause a rollback which makes the current session effectively dead, this also makes it
     * impossible to control the failure so instead we return null as a marker.
     * 
