...
public class RestServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        this.logic = wac.getBean(IClickerLogic.class);
        log.info("RestServlet:: logic: {}", logic);
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String method = "POST";
        String compensateMethod = req.getParameter(COMPENSATE_METHOD);

        if (StringUtils.isNotBlank(compensateMethod)) {
            // Allows override to GET or DELETE
            compensateMethod = compensateMethod.toUpperCase().trim();

            if (StringUtils.equals("GET", compensateMethod)) {
                method = "GET";
            } else if (StringUtils.equals("DELETE", compensateMethod)) {
                method = "DELETE";
            }
        }
    ...
    protected void handle(
            HttpServletRequest req, HttpServletResponse res, String method) throws ServletException, IOException {
        ...
        if (valid) {
            ...
            if (req.getParameter("_debug") != null || logic.isForceRestDebugging()) {
                restDebug = true;
        ...
        if (valid) {
            ...
            String msg = "ERROR " + status + ": Invalid request (" + req.getMethod() + " " + req.getContextPath() + req
                ...
                "                                 return 200 if valid OR 501 if SSO not enabled OR 400/401 if key" +
                "is bad \n" +
                "POST /register                 - Add a new clicker registration, return 200 for success or 400" +
                "with \n" +
                "                                 registration response (XML) for failure \n" + "GET  /courses   " +
                "               - returns the list of courses for the current user (XML) \n" +
                "GET  /students/{courseId}      - returns the list of student enrollments for the given " +
                "course (XML) \n" + "                                 or 403 if user is not an instructor in the " +
                "specified course \n" + "POST /gradebook/{courseId}     - send the gradebook data into the system," +
                "returns errors on failure (XML) \n" + "                                 or 'True' if no errors," +
                " 400 if the xml is missing or courseid is invalid, \n" + "                                 403 if" +
                " user is not an instructor in the specified course \n" + "\n" + " - Authenticate by sending " +
                "credentials (" + LOGIN + "," + PASSWORD + ") or by sending a valid session id (" + SESSION_ID +
                ") in the request parameters \n" + " -- SSO authentication requires an encoded key (" + SSO_KEY +
                ") in the request parameters \n" + " -- The response headers will include the sessionId when " +
                "credentials are valid \n" + " -- Invalid credentials or sessionId will result in a 401 (invalid " +
                "credentials) or 403 (not authorized) status \n" + " - Use " + COMPENSATE_METHOD + " to override" +
                " the http method being used (e.g. POST /courses?" + COMPENSATE_METHOD + "=GET will force the " +
                "method to be a GET despite sending as a POST) \n" + " - Send data as the http request BODY or as " +
                "a form parameter called " + XML_DATA + " \n" +
    ...
    private void handleAuthN(HttpServletRequest req, HttpServletResponse res) {
        ...
        if (auth != null && auth.getLoginName() != null) {
            ...
            sessionId = logic.authenticate(auth.getLoginName(), auth.getPassword(), ssoKey);

            if (sessionId == null) {
                throw new SecurityException("Invalid login credentials (" + LOGIN + "," + PASSWORD + ") supplied");
            }
        } else if (sessionId != null && sessionId.length() > 1) {
            boolean valid = logic.getExternalLogic().validateSessionId(sessionId, true);
    ...
    private String currentUser() {
        return logic.getExternalLogic().getCurrentUserId();
    }

    private boolean isAdmin(String userId) {
        return logic.getExternalLogic().isUserAdmin(userId);
    }

    private boolean isInstructor(String userId) {
        return logic.getExternalLogic().isUserInstructor(userId);
    }
